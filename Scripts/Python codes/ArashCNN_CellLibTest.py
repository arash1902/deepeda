...
from readMat_CellLib import CirRecDataset,next_batch
...
def compute_cost(Z3, Y):
    ...
    predict_op = tf.argmax(Z3, 1,name="op_to_restore")
    ### END CODE HERE ###
...
def numbers_to_strings(argument):
	...
	return switcher.get(argument, "mod operation")
def run_CNNCL(data_orig,labels):
	Input_shape = [None, 120, 28, 1]; 
	data = data_orig.reshape([-1, Input_shape[1], Input_shape[2],1])
	label =  np.fromstring(labels, dtype=int,sep=' ')
	label = label.reshape([1,label.shape[0]])
	label = label.astype(int)
	Y_train = convert_to_one_hot(label, 3).T
	conv_layers = {}
	#_, _, parameters = test(data, Y_train)
	...
	with tf.Session() as sess:
		saver = tf.train.import_meta_graph('my_conv_network.meta')
		...
		predict_op = sess.run(op_to_restore, {X: data, Y: Y_train, keep_prob1 : 1.0, keep_prob2 : 1.0})
		# print("CNN predict: ", predict_op)
		# print("\nreal operation is: ", label)
		predict= numbers_to_strings(int(predict_op[0]))
		operate= numbers_to_strings(int(label[0]))
		print '\nCNN predict: ' + predict
		print '\n real operation is: '+ operate
		#train_accuracy = accuracy.eval({X: data, Y: Y_train, keep_prob1 : 1.0, keep_prob2 : 1.0})	
### run it
def main():
# %matplotlib inline
	np.random.seed(1)
