...
import numpy as np
def CirRecDataset():
	path = '/home/home2/students/arash/ABC/src/ext/benchmarks/div_binCelllib/'
	datalist = [ ]
	...
	path = '/home/home2/students/arash/ABC/src/ext/benchmarks/mul_binCelllib/'
	for filename in os.listdir(path):
		filepath = os.path.join(path, filename)
		contents = open(filepath).read()
		readcontents = [item.split() for item in contents.split('\n')[:-1]]
	...
	path = '/home/home2/students/arash/ABC/src/ext/benchmarks/mod_binCelllib/'
	for filename in os.listdir(path):
		filepath = os.path.join(path, filename)
		contents = open(filepath).read()
		readcontents = [item.split() for item in contents.split('\n')[:-1]]
